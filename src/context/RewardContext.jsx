...
export function RewardProvider({ children }) {
  ...
  // Cost/reward amounts are no longer trusted from the caller — the
  // claim_puzzle_bubble RPC hardcodes them server-side and debits the
  // server-tracked repeated_pixels balance atomically. This just refreshes
  // local state from what the server actually returned.
  async function claimBubbleFromServer(bubbleId) {
    const { data, error } = await supabase.rpc('claim_puzzle_bubble', { p_bubble_id: bubbleId });
    if (error) {
      console.error('claim_puzzle_bubble failed:', error);
      return null;
    }
    const updated = Array.isArray(data) ? data[0] : data;
    setRewards(prev => ({ ...prev, key: updated.keys, chip: updated.chips, hourglass: updated.hourglasses, level: updated.level }));
    return updated;
  }

  // ── PUZZLE GRAND REWARD ──────────────────────────────────
  // Fired when the pixel board looks fully revealed on the client (see
  // PuzzleContext) — but that's just a hint to try claiming, not proof.
  // The actual gate is server-side: claim_puzzle_grand_reward only pays
  // out once per ~29-day window per account (tracked by
  // profiles.last_puzzle_claim_at), independent of anything the client
  // claims about board state. This replaces the old static-task-id
  // approach, which could only ever pay out once per account, ever.
  async function grantPuzzleComplete() {
    if (!user) return;

    const { data, error } = await supabase.rpc('claim_puzzle_grand_reward');
    if (error) {
      // Expected once the reward was already claimed this cycle — not a bug.
      if (!/already claimed/i.test(error.message ?? '')) {
        console.error('claim_puzzle_grand_reward failed:', error);
      }
      return;
    }

    const updated = Array.isArray(data) ? data[0] : data;
    setRewards({ key: updated.keys, chip: updated.chips, hourglass: updated.hourglasses, level: updated.level });
    setCompletedTasks(new Set(updated.completed_tasks ?? []));
    window.dispatchEvent(new CustomEvent('chips-updated'));

    const { chips, keys, hourglasses } = PUZZLE_GRAND_REWARD;
    if (chips > 0) flyReward({ type: 'chip' });
    if (keys > 0) setTimeout(() => flyReward({ type: 'key' }), 200);
    if (hourglasses > 0) setTimeout(() => flyReward({ type: 'hourglass' }), 400);
